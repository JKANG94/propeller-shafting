...
for nn=1:9
    ...
    subplot(3,3,nn) 
    [hengmatrix,zongmatrix,~,heng2,zong2,~]=transfer_matrix(spring1,spring2,L01,L02,d1,d2,d3,2*pi*result_fre(nn),density);
%     [hengmatrixw,~,~,heng2w,~,~]=transfer_matrix(springw,L01,L02,d1,d2,d3,2*pi*result_fre(nn),density);
...
%     zlabel('垂向模态位移/m')
%     legend('固有结构','模态振型');
    text(0.1,0.1,0.1,[num2str(result_fre(nn)),'Hz']);
    view(3)
